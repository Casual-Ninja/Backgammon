...
namespace GAME
{
    ...
    public class BackGammonChanceState : State
    {
        ...
        public override MessageType messageType
        {
            get
            {
                return MessageType.ChanceState;
            }
        }
        ...
        public static List<(sbyte, sbyte)> GetLegalActionsOfOneDie(byte theDie, BackGammonChoiceState prevState)
        {
            BackGammonChoiceState state = new BackGammonChoiceState((BackGammonChoiceState)prevState);
            List<(sbyte, sbyte)> legalActions = new List<(sbyte, sbyte)>();
            ...
            if (state.myEatenCount == 0)
            {
                ...
                for (int i = 0; i < state.myPieces.Count; i++)
                {
                    if (state.myPieces[i] + theDie < 24)
                    {
                        int addToIndex = (state.board[state.myPieces[i]] == 1) ? 1 : 0;
                        if (state.board[state.myPieces[i] + theDie] >= -1)
                        {
                            // can move this piece
                            bool isEndGame2 = isEndGame ||
                                (i == 0 && state.board[state.myPieces[0]] == 1 && state.myPieces[0] + theDie >= 18 && (state.myPieces.Count == 1 || state.myPieces[1] >= 18));

                            legalActions.Add(((sbyte)state.myPieces[i], (sbyte)(state.myPieces[i] + theDie)));
                        }
                    }
                    else
                    {
                        if (isEndGame && (i == 0 || state.myPieces[i] + theDie == 24))
                        {
                            legalActions.Add(((sbyte)state.myPieces[i], -1));
                        }
                        break;
                    }
                }
            ...
            {
                if (state.board[theDie - 1] >= -1)
                {
                    legalActions.Add((-1, (sbyte)(theDie - 1)));
                }
            }
        ...
        public BackGammonChoiceState MoveWithoutRotate(BackGammonChoiceState prevState, BackGammonChanceAction action)
        {
    ...
    public class BackGammonChanceAction : Action
    {
        ...
        public bool ContainsMovement((sbyte, sbyte) move)
        {
            for (int i = 0; i < Count; i++)
                if (indexes[i] == move)
                    return true;
